...
class Blocks
{
	private $table_name = "block_users";
	...
	function getByIpBlocks($ip){

		$data = $this->connection->select($this->table_name, '*', " WHERE ip='$ip' ");
		return $data;
	}
	
	function getByFbBlocks($fb){

		$data = $this->connection->select($this->table_name, '*', " WHERE facebook_id='$fb' ");
		return $data;
	}
